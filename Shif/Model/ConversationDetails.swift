...
struct Api_Conversation : Codable {
    let result : [Res_Conversation]?
    let status : String?
    let message : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_Conversation].self, forKey: .result)
        status = try values.decodeIfPresent(String.self, forKey: .status)
        message = try values.decodeIfPresent(String.self, forKey: .message)
    }
}

struct Res_Conversation : Codable {
    let id : String?
    let store_id : String?
