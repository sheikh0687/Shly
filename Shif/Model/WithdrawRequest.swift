...
struct Api_WithdrawRequest : Codable {
    let result : Res_WithdrawRequest?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_WithdrawRequest.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_WithdrawRequest : Codable {
    let id : String?
    let user_id : String?
    let amount : String?
    let branch : String?
