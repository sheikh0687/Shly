...
struct Api_ChangeStatus : Codable {
    let result : Res_ChangeStatus?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_ChangeStatus.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_ChangeStatus : Codable {
    let id : String?
    let user_id : String?
