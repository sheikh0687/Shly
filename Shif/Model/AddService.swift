...
struct Api_AddService : Codable {
    let result : Res_AddService?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_AddService.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_AddService : Codable {
    let id : String?
    let user_id : String?
    let cat_id : String?
    let cat_name : String?
