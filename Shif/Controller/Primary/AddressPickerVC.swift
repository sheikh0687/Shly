...
class AddressPickerVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if is_Coming == "Signup" || is_Coming == "ProviderManageProfileVC" {
            self.address_TypeVw.isHidden = true
        } else {
            self.address_TypeVw.isHidden = false
        }
    }
    ...
    @IBAction func btnSubmitAddress(_ sender: UIButton) {
        if is_Coming == "Signup" || is_Coming == "ProviderManageProfileVC" {
            ...
            if self.txtAddressType.text == "" || self.txtSearchLocation.text == "" {
    ...
    func addAddress() {
        Api.shared.addAddress(self, paramDetails()) { responseData in
            self.dismiss(animated: true) {
                self.locationPickedBlock?(self.location_cordinate ?? kAppDelegate.coordinate2.coordinate, self.lat ?? kAppDelegate.coordinate2.coordinate.latitude, self.lon ?? kAppDelegate.coordinate2.coordinate.longitude, self.address)
            }
        }
    ...
    func paramDetails() -> [String : AnyObject] {
        ...
        dict["addresstype"]            = self.txtAddressType.text as AnyObject
